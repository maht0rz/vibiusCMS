{{$extends_rpdev/admin/layouts/basic}}
  ...
  {{$section_sidebar}}
    ...
    <li class="active"><a href="{{$url}}admin/dashboard/styles"><span class="fa fa-tags faicon"></span>Assets Manager</a></li>
...
{{$sectionEnd_cnav}}
  ...
  {{$section_javascript}}
    ...
    function getPages(){
        ...
        $('#fileTree').html('');
      $.get("{{$url}}admin/ajax/getStylesheet",function(data,status){
        console.log(data);
           json = jQuery.parseJSON(data);

           $('#fileTree').append(
             '<li><a id="add-new-stylesheet" href="#"><span class="fa fa-plus faicon"></span><span class="mtext">Add new stylesheet</span></a></li>'
           );
...
post_type = 'css';
            ...
            $.get("{{$url}}admin/ajax/getfileStyle/"+file,function(data,status){
              console.log(data);
              json = jQuery.parseJSON(data);
...
current_file = $('#pagename').val();
      ...
      $.get("{{$url}}admin/ajax/getJavascript",function(data,status){
        console.log(data);
           json = jQuery.parseJSON(data);

           $('#fileTree').append(
             '<li><a id="add-new-javascript" href="#"><span class="fa fa-plus faicon"></span><span class="mtext">Add new javascript</span></a></li>'
           );
        ...
        $('.folder-pages').on('live click', function(){
          post_type = 'js';
            ...
            $(this).addClass('menu-selected');
            $('#curfile').text($(this).data('file'));
            $.get("{{$url}}admin/ajax/getfileJavascript/"+file,function(data,status){
              console.log(data);
              json = jQuery.parseJSON(data);
              current_file = json.fileName;
              $('#pagename').val(json.name);
        ...
        $('#add-new-javascript').on('live click', function(){
          post_type = 'js';
            current_file = false;
            old_file = '';
            editor.getSession().setValue('');
      ...
      $(document).ready(function(){
        getPages();
          ...
          $('#save').click(function(){
              ...
              if(post_type == 'js'){
                var post_url = "{{$url}}admin/ajax/updateScript";
              }else{
                var post_url = "{{$url}}admin/ajax/updateStyle";
              }
